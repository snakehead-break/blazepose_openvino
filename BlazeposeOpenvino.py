...
class BlazeposeOpenvino:
    ...
    def lm_render(self, frame, region):
        ...
        if region.lm_score > self.lm_score_threshold:
            ...
            if self.show_landmarks:
                ...
                resultkiri = abs(AngleBtw2Points(lines[0][6],[x3,y3]))
                resultkanan = abs(AngleBtw2Points([x3,y3],lines[0][7]))
                result = abs(self.regresi.hitungprediksi([resultkiri,resultkanan]))
    ...
    def run(self):
        ...
        while True:
            ...
            cv2.imshow("Sistem Posture Check", annotated_frame)
            ...
            if key == ord('q') or key == 27:
                ...
                z2 = pygetwindow.getActiveWindow().title
                scWindow = pygetwindow.getWindowsWithTitle(z2)[0]
                print(scWindow)
                im = pyscreenshot.grab(bbox=(int(scWindow.left), int(scWindow.top),int(scWindow.width)+150,int(scWindow.height)+150))
                im.save(f'screenshot/scUser{n:1f}.png')
                n =+ 1
...
class regression:
    def hitungregresi(self):
        df = pd.read_csv("Book1.csv", header=0, sep=",")
        df['delta_derajat_bawah'] = abs(df.loc[:, 'KIRI BAWAH'] - df.loc[:, 'KANAN BAWAH'])
        df['delta_derajat_atas'] = abs(df.loc[:, 'KIRI ATAS'] - df.loc[:, 'KANAN ATAS'])
        df1 = pd.read_csv("Book2.csv", header=None, sep=",")
        df1['avg_derajat'] = np.average(df1.iloc[:, 1:], axis=1)
        model = LinearRegression()
        y = df['delta_derajat_bawah'].values.reshape(-1, 1)  # Mengubah dimensi menjadi array 2D
        x = df[['KIRI BAWAH', 'KANAN BAWAH']].values  # Mengambil dua kolom sebagai data latih
    ...
    def hitungprediksi(self, data_asli):
        model_regresi = self.hitungregresi()

        # Contoh data uji berisi satu pasang koordinat bahu kiri dan bahu kanan [x1, y1, x2, y2]
        data_uji = np.array(data_asli)  
        # Misalnya, data ini merupakan satu pasang koordinat bahu kiri dan bahu kanan

        # Ubah dimensi data_uji menjadi array 2D
        data_uji = data_uji.reshape(1, -1)
        # Lakukan prediksi
        hasil_prediksi = model_regresi.predict(data_uji)
